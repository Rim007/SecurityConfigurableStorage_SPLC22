...
We intentionally excluded papers that do not address security, SPL engineering, or a data storage medium (\ref{it:icFocus}), e.g., publications proposing actual configurable SaaS systems, including a storage medium and its security concerns but without any reference to SPL engineering.
\todo{example}
...
Overall, we defined the following extraction criteria classified into four thematic categories:\smallskip
...
\noindent\textbf{Security:}	  
    \begin{itemize}[nosep]
	\item \textbf{Standard} a publication refers to (e.g., ISO 27001~\cite{ISOIEC27001:2013}, IEC 62443~\cite{IEC62443}).
	...
	\item \textbf{Mitigation techniques} proposed or implemented to mitigate the impact of cyber-attacks and to achieve certain security goals (e.g., Intel Software Guard Extensions~\citep{Costan2016IntelExplained}).
	\end{itemize}\smallskip
...
\noindent\textbf{Configurable system:}	  
	...
	\item \textbf{Tool support} reported in the publication. (e.g., pure::variants~\cite{Beuche2012purevariants}, FeatureIDE~\cite{Meinicke2017FeatureIDE}, own tools).
\end{itemize}
...
However, after reading the full-texts, we had to remove 33 paper since they only superficially dealt with SPL and data storage or only mentioned these aspects in the context of related work.

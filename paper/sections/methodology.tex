...
So, following search string we defined by the first two authors:
...
To extract data in a systematic way, the first and the second author defined categories covering the two relevant areas security and configurable data storages.
More specifically, we based the extraction categories on related papers, particularly the related mapping study of \citet{Kenner2021MappingStudy} and the papers referenced therein.
...
After integrating the papers into the literature review tool Rayyan QCRI,\footnote{\url{www.rayyan.ai}} we labeled each paper as included, excluded, or duplicate (i.e., removing 29 duplicates).
Then, our selection criteria were employed on titles and abstracts, resulting in 76 papers for the full-text analysis. 
After reading the full-texts, 32 more papers were removed, since they only superficially dealt with configurable data storages or mentioned these aspects only in the context of related work.
Then, one iteration of backwards snowballing was employed on the remaining 44 papers.
Finally, we identified 50 papers to be relevant and to extract data based on the defined categories.
...
This way, we classified recurring information in the extracted data and synthesized common themes for each data category.
In this context, created categories, extraction results, and subsequent interpretations of the results (i.e., research opportunities) were derived and adjusted based on discussions with the second author.

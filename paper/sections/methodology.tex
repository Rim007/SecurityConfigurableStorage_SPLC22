...
We note that our previous mapping study on safety and security for configurable systems~\citep{Kenner2021MappingStudy} only focused on configurable software systems but did not considered storage systems in a certain way. \todo{SPLC is double blind this year I think? so would be needed to be in third person. but its easy to rephrase, I would basically say that from last year's SPLC we are aware of this paper but our goals are actually different and we cover another body of work (i.e., papers)}
...
We tried \todo{don't try, do or don't ;) We considered keywords that cover a wide range of ... with the goal of obtaining an overview of different types of architectures --- something like this, basically means the same but less attackable} to cover a wide range of different storage systems, without any restriction regarding their architecture (e.g., centralized or decentralized), including solutions ranging from databases to service-based cloud storage.
...
\todo{hm I think it would be good to have a few details on which keywords have been selected for what purpose (i.e., what do they cover)}
...
These literature databases ensure a certain quality by indexing only peer-reviewed publications from a variety of publishers \todo{but the last point does not account for IEEE xplore right?}).
...
In this context, we argue that publications older than ten years have usually already become established practices,  standards, or are no longer relevant. \todo{hm not sure about the relevancy/... arguments in this case, maybe its better to just say focusing on recent advances and limiting the number of publications, or at least providing some intuition why the argument is likely to be true}
Since we did not perform a detailed quality assessment of all selected papers, we tried to ensure a certain quality by relying on the review of publication venues by \textsc{Scopus}, \textsc{IEEE}, and \textsc{ACM} (\ref{it:icPaper}). \todo{to hard, would rather say that such criteria are not applicable since we cover a variety of works that cannot be properly compared against each other and only bring this argument as an additional point}
...
\todo{a few more details, namely that you did this (ensures consistency), how many duplicates were in, how was the data extracted stored and synthesized (e.g., open-coding like process storing it in a speardsheet and synthesis via open-card sorting?) --- but this actually belongs in the methodology}
...
\todo{I think the number of papers is fine ... 53 is quite a lot. but it would still be good to have a somewhat convincing argument why we consider this to be enough and did not employ additional searches}

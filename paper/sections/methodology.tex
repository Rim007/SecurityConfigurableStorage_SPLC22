...
\begin{itemize}[nosep]
    ...
    \begin{itemize}
	    \item \textit{Security $\rightarrow$ SPL}: publications focusing on the security of a configurable system
		\item \textit{SPL $\rightarrow$ security}: publications addressing a security concern based on product-line techniques
		\item \textit{Storage $\rightarrow$ SPL}: publications focusing on storage systems used to support certain SPL tasks (e.g., storing variants)
		\item \textit{SPL $\rightarrow$ Storage}: publications focusing on storage systems based on product-line techniques
	\end{itemize}
	...
	\item \textbf{Stored data type} indicating whether or which data is (securely) stored in storage solution
	\item \textbf{Security standard} a publication refers to (e.g., ISO 27001)
	\item {Security goals} described by a publication 
	\begin{itemize}
	    \item \textit{CIA triad}, including confidentiality, integrity, and availability
	    \item \textit{Information security}, including authorization, accountability, and non-repudiation
	\end{itemize}
	...
	\item \textbf{Security technology} implemented or proposed to achieve certain security objectives (e.g., Intel Software Guard Extensions~\citep{Costan2016IntelExplained})
	...
	\item \textbf{Evolution} of a configurable storage system

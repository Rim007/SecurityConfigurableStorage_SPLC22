...
After describing the extraction results, we give an overview of relevant core insights and \textbf{literature gaps (LG)} derived from the results after collaboratively analyzing and discussing the results among this study's authors.
...
In detail, storage is either referred to as medium (e.g., a database), the environment of the medium (e.g., a configurable cloud system), or the cooperation of both, usually with a focus on the environment. 
...
Our findings also emphasize that \textbf{(LG\textsubscript{5})} \emph{there is a lack of concrete specifications of mitigation techniques} (e.g., concrete encryption algorithms, such as AES-256 or RSA-2048).
...
Thus, we state that \textbf{(LG\textsubscript{6})} \emph{a concrete assignment of mitigation techniques to their target area (i.e., its perspective) is usually missing}, leading to a decreasing comparability of existing approaches.
...
However, we emphasize that \textbf{(LG\textsubscript{7})} \emph{concrete security goals (e.g., according to the definitions given in ISO/IEC 27000~\cite{ISOIEC270002018}) are usually missing in the context of configurable systems}, implying a lack of practical orientation.
...
However, if the solution space is not sufficiently addressed, \textbf{(LG\textsubscript{8})} \emph{sufficient concrete approaches that can serve as working examples for practice are missing}. 
...
Therfore, we argue that \textbf{(LG\textsubscript{9})} \emph{there is a lack of research in collaboration with practitioners (e.g., industry partners)}.
...
Furthermore, we found out that \textbf{(LG\textsubscript{12})} \emph{tools to support the product-line engineering process are usually missing}.
...
Nevertheless, these considerations also lead to the issue that \textbf{(LG\textsubscript{13})} \emph{security is usually only addressed from a high-level perspective}, leading to a lack of concrete security measures.
...
This is why we argue that \textbf{(LG\textsubscript{14})} \emph{more research is needed in the context of variable storage mediums, i.e., databases, to understand relevant relationships and to minimize security risks cause by storage variability}.
...
Nevertheless, this issue shows that \textbf{(LG\textsubscript{15})} \emph{the treatment of security threats or risks caused by variability and the relationship between variability and the emergence of vulnerabilities need more research}.
...
However, \textbf{(LG\textsubscript{16})} \emph{the research area of configurable systems, especially configurbable storages, and security seems under-explored} since storage is usually interpreted as a part of a system rather than as an individual system with requirements regarding variability.

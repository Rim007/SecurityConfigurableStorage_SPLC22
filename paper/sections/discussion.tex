...
\myPar{Storage}
database nicht genau definiert (SQL, NO SQL), müssen die einzeln gesichert werden oder entsprechende Anforderungen je nach Datenbanktyp betrachtet werden.
wie relevant ist die Version oder Art einer Datenbank, was passiert, wenn geupdated wird, welchen Einfluss hat Variabilität, Unterscheidung DBMS und DB, inwiefern sind automatismen in DB eine Gefärdung, wie verhält sich das bei variablen Systemen, wie sieht das aus im Kontext der Variabilität (SQLite)
Organisationsstruktur (centralized, decentralized, distributed)
Unterscheidung medium, environment (Cloud ist nicht gleich speichermedium) --> storage wird als überbegriff aufgefasst

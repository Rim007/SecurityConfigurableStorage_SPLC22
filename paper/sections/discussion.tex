...
Nevertheless, we recommend to do more further research in the context of the organizational structure of configurable systems and their storage, e.g. defining the layers in terms of a framework.
...
We propose the definition of different protection levels and assigned security measures, depending on which data, medium, environment, or system (including variants) need to be protected (e.g., different need for protection of personal data such as birthdate or medical data such as medical history).
...
Thus, we state that \textbf{(LG\textsubscript{7})} \emph{a concrete assignment of mitigation to its target area (i.e., its perspective) is usually missing}, leading to a decreasing comparability of existing approaches.

Surprisingly, the security goals of the CIA triad (confidentiality, integrity, availability) were not addressed the most, as in the mapping study of \citet{Kenner2021MappingStudy}.
...
Referring to the projection of the examined systems, most publications seem to focus on the mapping between problem space and solution space or the problem space, meaning that actual (implemented) configurable systems are not published sufficiently.
However, if the solution space is not sufficiently addressed, \textbf{(LG\textsubscript{9})} \emph{there are not enough concrete approaches that can serve as working examples for practice are missing}. 
Furthermore, we could hardly find any publications that were published with industry partners (e.g., in the production domain).
...
Addressing this issue could lead to an increasing the confidence in configurable systems and their data storage, including a higher trust in feature interactions.
Moreover, costs could be avoided by avoiding potential system adaptions~\cite{Kenner2021MappingStudy}, e.g., in terms of updates, which could also lead to new security risks.
...
We assume that this fact is maybe due to the papers' focus on cloud systems which are possibly subject of more modifications due to their high scalability according to the systems' requirements.
...
Not surprisingly, most of the analyzed systems consider security as non-functional requirement, quality attribute, or overall system goal. 
...
We identified that the databases described in the publications are usually not explained in detail, e.g. whether they are SQL or NoSQL databases or how variable they actually are.
...
Interestingly, some publications stated variability as one issue that could threaten the security of a configurable system or its storage.
However, although this threat was recognized, only in one case a concrete mitigation technique was proposed, precisely the parallel execution variants~\cite{nguyen2014exploring}).
Nevertheless, this issue shows that \textbf{(LG\textsubscript{16})} \emph{the treatment  of security threats or risks caused by variability and the relationship between variability and the emergence of vulnerabilities seems underexplored}.
